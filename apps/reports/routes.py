...
@blueprint.route("/profiles_report")
@login_required
def profiles_report():
    page = request.args.get("page", 1, type=int)
    per_page = 50  # Number of logs per page
    ...
    # filter influencers
    profiles = (Profile.query.filter(Profile.name.ilike(f"%{search_terms}%"))).paginate(
        page=page, per_page=per_page, error_out=False
    )
    
    report_data = []
    for profile in profiles:
        # Query the last Profile_Actions entry for the current profile
        last_profile_action = (
            Profile_Actions.query
            .filter_by(profile_id=profile.id)
            .order_by(Profile_Actions.creation_date.desc())
            .first()
        )

        if last_profile_action:
            now = datetime.datetime.now().date()
            difference = (now - last_profile_action.creation_date).days
            rem_days = last_profile_action.action.due_days - difference
            alert = (last_profile_action.action.alert if last_profile_action.action else 'لا يوجد') if rem_days<=0 else 'لا يوجد'
            establishment = last_profile_action.action.establishment if last_profile_action.action else None

            report_data.append({
                'profile_id':profile.id,
                'profile_code':profile.code,
                'profile_name': profile.name,
                'profile_creation_date':profile.creation_date,
                'last_action':last_profile_action.action.name,
                'last_creation_date': last_profile_action.creation_date,
                'establishment': establishment,
                'rem_days': rem_days,
                'alert': alert
            })
        else:
            # Handle profiles with no actions
            report_data.append(
                {
                    "profile_id": profile.id,
                    "profile_code": profile.code,
